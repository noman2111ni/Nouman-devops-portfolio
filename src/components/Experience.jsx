...
function TimelineItem({ year, role, company, type, color, points, index }) {
  ...
  const Card = () => (
    <div style={{
      padding:"24px 22px", background:"rgba(15,23,42,0.02)",
      ...
      <h3 style={{ fontWeight:800, fontSize:"1rem", color:"#0f172a", marginBottom:4 }}>{role}</h3>
      <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.78rem", color:"#475569", marginBottom:14 }}>{company}</div>
      <ul style={{ listStyle:"none" }}>
        {points.map((p,i) => (
          <li key={i} style={{ fontSize:"0.84rem", color:"rgba(15,23,42,0.62)", marginBottom:7, display:"flex", gap:8 }}>
  ...
  if (isMobile) return (
    <div ref={ref} style={{
      ...
      <div style={{ display:"flex", flexDirection:"column", alignItems:"center", flexShrink:0 }}>
        <div style={{ flex:1, width:1, background:"rgba(15,23,42,0.08)" }} />
    ...
    <section id="experience" style={{ padding: isMobile?"80px 24px":"110px 60px" }}>
      <div style={{ maxWidth:1100, margin:"0 auto" }}>
        <div ref={ref} style={{ marginBottom:70, textAlign:"center", opacity:visible?1:0, transform:visible?"translateY(0)":"translateY(24px)", transition:"all 0.7s ease" }}>
          <p style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.72rem", letterSpacing:"0.35em", color:"#2563eb", textTransform:"uppercase", marginBottom:12 }}>// Experience</p>
          <h2 style={{ fontSize:"clamp(2rem,5vw,3.5rem)", fontWeight:800, lineHeight:1.1 }}>
            My <span style={{ background:"linear-gradient(135deg,#7c3aed,#f97316)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }}>Journey</span>
          </h2>
        </div>
