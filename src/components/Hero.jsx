  ...
  return (
    <section id="hero" style={{
      ...
      {!isMobile && <div style={{
        ...
        background: `radial-gradient(600px at ${mouse.x}px ${mouse.y}px, rgba(37,99,235,0.08), transparent 60%)`,
      }} />}
      <div style={{ position: "absolute", inset: 0, pointerEvents: "none",
        background: "radial-gradient(ellipse 80% 60% at 50% 50%, rgba(124,58,237,0.08) 0%, transparent 65%)",
      }} />
      ...
      <div style={{ position: "relative", zIndex: 2, textAlign: "center", maxWidth: 900, width: "100%" }}>
        <div style={{
          ...
          border: "1px solid rgba(37,99,235,0.28)", padding: "7px 18px",
          marginBottom: 28, background: "rgba(37,99,235,0.05)", borderRadius: 2,
          animation: "fadeDown 0.8s ease both",
        }}>
          <span style={{ width:7,height:7,borderRadius:"50%",background:"#22c55e",boxShadow:"0 0 8px rgba(34,197,94,0.7)",animation:"pulse 2s infinite" }} />
          Available for Hire
        </div>
        ...
        <div style={{
          ...
          fontWeight: 300, color: "rgba(15,23,42,0.72)",
          animation: "fadeDown 0.8s 0.3s ease both", animationFillMode: "both", minHeight: "2rem",
        }}>
          I am a <span style={{ color:"#f97316", fontWeight:700 }}>
            {typed}<span style={{ animation:"blink 1s infinite", color:"#2563eb" }}>|</span>
          </span>
        </div>

        <p style={{
          marginTop: 24, fontSize: isMobile ? "0.88rem" : "1.05rem",
          lineHeight: 1.85, color: "rgba(15,23,42,0.6)",
        ...
        <div style={{
          ...
          {[["View Projects","#projects",true],["Contact Me","#contact",false]].map(([label,href,primary]) => (
            <button key={label} onClick={() => handleScroll(href)} style={{
              ...
              border: primary ? "none" : "1px solid rgba(37,99,235,0.35)", cursor: "pointer",
              clipPath: "polygon(8px 0%,100% 0%,calc(100% - 8px) 100%,0% 100%)",
              transition: "all 0.3s",
            }}
              onMouseEnter={e => { e.target.style.background = primary ? "#f97316" : "rgba(37,99,235,0.08)"; e.target.style.transform = "translateY(-3px)"; }}
              onMouseLeave={e => { e.target.style.background = primary ? "#2563eb" : "transparent"; e.target.style.transform = "translateY(0)"; }}
            >{label}</button>
          ))}
        </div>
        ...
        <div style={{
          ...
          {['🐳 Docker','⚓ K8s','☁️ AWS','⚙️ Terraform','🔄 CI/CD','📊 Grafana'].map(t => (
            <span key={t} style={{
              ...
              color:"rgba(15,23,42,0.38)", letterSpacing:"0.08em", transition:"color 0.3s", cursor:"default",
            }}
              onMouseEnter={e => e.target.style.color="#2563eb"}
              onMouseLeave={e => e.target.style.color="rgba(15,23,42,0.38)"}
            >{t}</span>
          ))}
        </div>
      </div>
      ...
      <div style={{
        ...
        <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.6rem", letterSpacing:"0.28em", color:"#475569", textTransform:"uppercase" }}>Scroll</span>
        <div style={{ width:1, height:44, background:"linear-gradient(to bottom, #2563eb, transparent)", animation:"scrollLine 2s ease-in-out infinite" }} />
      </div>
    </section>
  );
}
