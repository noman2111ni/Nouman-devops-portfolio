  ...
  return (
    <section id="hero" style={{
      ...
      <div style={{
        ...
        backgroundImage: `linear-gradient(rgba(0,132,217,0.04) 1px, transparent 1px),linear-gradient(90deg, rgba(0,132,217,0.04) 1px, transparent 1px)`,
        backgroundSize: "60px 60px", animation: "gridMove 20s linear infinite",
      }} />
      {!isMobile && <div style={{
        position: "absolute", inset: 0, pointerEvents: "none",
        background: `radial-gradient(600px at ${mouse.x}px ${mouse.y}px, rgba(0,132,217,0.06), transparent 60%)`,
      }} />}
      ...
      <div style={{ position: "relative", zIndex: 2, textAlign: "center", maxWidth: 900, width: "100%" }}>
        <div style={{
          ...
          color: "#0084d9", textTransform: "uppercase",
          border: "1px solid rgba(0,132,217,0.3)", padding: "7px 18px",
          marginBottom: 28, background: "rgba(0,132,217,0.05)", borderRadius: 2,
        ...
        <h1 style={{
          ...
          background: "linear-gradient(135deg, #1a1a1a 0%, #0084d9 50%, #7c3aed 100%)",
        ...
        <div style={{
          ...
          fontWeight: 300, color: "rgba(26,26,26,0.55)",
          ...
          I am a <span style={{ color:"#ff6b35", fontWeight:700 }}>
            {typed}<span style={{ animation:"blink 1s infinite", color:"#0084d9" }}>|</span>
          </span>
        </div>

        <p style={{
          marginTop: 24, fontSize: isMobile ? "0.88rem" : "1.05rem",
          lineHeight: 1.85, color: "rgba(26,26,26,0.42)",
        ...
        <div style={{
          ...
          {[["View Projects","#projects",true],["Contact Me","#contact",false]].map(([label,href,primary]) => (
            <button key={label} onClick={() => handleScroll(href)} style={{
              ...
              background: primary ? "#0084d9" : "transparent",
              color: primary ? "#ffffff" : "#0084d9",
              fontFamily: "'JetBrains Mono',monospace",
              fontSize: isMobile ? "0.72rem" : "0.82rem",
              fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase",
              border: primary ? "none" : "1px solid rgba(0,132,217,0.4)", cursor: "pointer",
              clipPath: "polygon(8px 0%,100% 0%,calc(100% - 8px) 100%,0% 100%)",
              transition: "all 0.3s",
            }}
              onMouseEnter={e => { e.target.style.background = primary ? "#ff6b35" : "rgba(0,132,217,0.08)"; e.target.style.transform = "translateY(-3px)"; }}
              onMouseLeave={e => { e.target.style.background = primary ? "#0084d9" : "transparent"; e.target.style.transform = "translateY(0)"; }}
            >{label}</button>
          ))}
        </div>
        ...
        <div style={{
          ...
          {["🐳 Docker","⚓ K8s","☁️ AWS","⚙️ Terraform","🔄 CI/CD","📊 Grafana"].map(t => (
            <span key={t} style={{
              ...
              color:"rgba(26,26,26,0.28)", letterSpacing:"0.08em", transition:"color 0.3s", cursor:"default",
            }}
              onMouseEnter={e => e.target.style.color="#0084d9"}
              onMouseLeave={e => e.target.style.color="rgba(26,26,26,0.28)"}
            >{t}</span>
          ))}
        </div>
      </div>
      ...
      <div style={{
        ...
        <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.6rem", letterSpacing:"0.28em", color:"#6b7280", textTransform:"uppercase" }}>Scroll</span>
        <div style={{ width:1, height:44, background:"linear-gradient(to bottom, #0084d9, transparent)", animation:"scrollLine 2s ease-in-out infinite" }} />
      </div>
    </section>
  );
}
